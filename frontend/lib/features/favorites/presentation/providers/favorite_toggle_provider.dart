...
class FavoriteToggleNotifier extends Notifier<AsyncValue<bool>> {
  ...
  Future<void> toggle() async {
    ...
    try {
      ...
      if (currentState) {
        ...
        result.fold((failure) {
          AppLogger.error('Failed to remove favorite $mediaId', failure);
          if (!_disposed) _syncFromProvider();
        }, (_) => favorites.removeLocal(mediaId));
      } else {
        final result = await addFavorite(mediaId);
        result.fold((failure) {
          AppLogger.error('Failed to add favorite $mediaId', failure);
          if (!_disposed) _syncFromProvider();
        }, favorites.addLocal);
      }
    } catch (e) {
      AppLogger.error('Failed to toggle favorite $mediaId', e);
      if (!_disposed) _syncFromProvider();
    } finally {
      _isToggling = false;
    }
  ...
  Future<bool> _isFavorited() async {
    ...
    if (favorites != null) {
      return favorites.any((f) => f.mediaId == mediaId);
    }
    try {
      final ids = await ref.read(favoriteMediaIdsProvider.future);
      return ids.contains(mediaId);
    } catch (e) {
      AppLogger.error('Failed to read favorite state $mediaId', e);
      return false;
    }
  }
}
