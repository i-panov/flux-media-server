...
class FakeMediaRepository implements MediaRepository {
  ...
  }) async => const Right(UploadResult(jobId: 0));
  ...
  Future<Either<Failure, Media>> updateMetadata(
    int mediaId,
    MetadataEdit edit,
  ) async => Right(_fakeMedia(mediaId));
  ...
  Future<Either<Failure, WatchProgress>> updateProgress(
    ...
    bool? completed,
      ...
      const Right(WatchProgress(id: 0, userId: 0, mediaId: 0, position: 0));
...
void main() {
  group('MediaListNotifier', () {
    ...
    test(
      'network failure marks the app offline (internet lost after launch)',
      () async {
    ...
    test(
      'loadMore failure keeps existing items but surfaces the error',
      () async {
        final first = [_fakeMedia(1), _fakeMedia(2)];
        fakeRepo.onGetMediaList = ({type, year, q, limit, offset}) async =>
            Right((items: first, total: 4));
    ...
    test('loadMore result is discarded when query changes', () async {
      ...
      final loadMoreFuture = container
          .read(mediaListProvider('video').notifier)
          .loadMore();
    ...
    test('loadMore at the end of the list does not make a request', () async {
      fakeRepo.onGetMediaList = ({type, year, q, limit, offset}) async =>
  ...
  group('MediaDetailNotifier', () {
    ...
    setUp(() async {
      ...
      container = ProviderContainer(
        overrides: [
          ...
          getMediaDetailUseCaseProvider.overrideWithValue(
            GetMediaDetail(fakeRepo),
          ),
        ],
      );
      fakeCache = container.read(
    ...
    Future<void> waitForDetailState(
      ...
      final sub = container.listen<MediaDetailState>(mediaDetailProvider(id), (
        _,
        next,
      ) {
    ...
    test('load emits error on failure', () async {
      fakeRepo.onGetMediaDetail = (_) async =>
    ...
    test('refresh keeps state and updates from server', () async {
      ...
      final sub = container.listen<MediaDetailState>(mediaDetailProvider(1), (
        _,
        next,
      ) {
      ...
      final updated = _fakeMedia(
        1,
        'New',
      ).copyWith(updatedAt: DateTime.utc(2025));
    ...
    test('refresh failure keeps the current loaded state', () async {
      fakeRepo.onGetMediaDetail = (_) async => Right(_fakeMedia(1, 'Old'));
